updateBidForPlace = async (rentalPlaceID, bidAmt) => {
    const {accounts, contract} = this.state;
	bidAmt *= 1000;
    this.setState ({loading: true});
    contract.methods
      .updateBid (rentalPlaceID)
      .send ({from: accounts[0], value: bidAmt.toString ()})
  ...
  closeBiddings = async (rentalPlaceID, toSend) => {
	const {accounts, contract} = this.state;
    this.setState ({loading: true });
    contract.methods
      .closeBiddings (rentalPlaceID)
      .send ({from: accounts[0], value: toSend.toString ()})
      .once ('receipt', receipt => {
        this.setState ({loading: false});
        this.getrentalPlaces ();
        this.setState ({redirect: "/AvailableRentals"});
  ...
  getMyTotalBid = async (rentalPlaceID) => {
	const {accounts, contract} = this.state;
    const myTotalBid = await contract.methods.getBiddersBid(rentalPlaceID).call();
	console.log("myTotalBid", myTotalBid)
    return myTotalBid;
  }
