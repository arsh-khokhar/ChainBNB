...
class RentalPlaces extends Component {
  ...
  async populateOccupancy(rentalPlaceID) {
    console.log("populating occupancy", rentalPlaceID);
                    ...
                    <div>
                      ...
                      {rentalPlace.placeOwner == this.props.activeAccount ? (
                        ...
                        <label>
                          ...
                          <div>
                            <label><strong> This place also has ongoing bidding for dates {rentalPlace.biddingStartDate + " to "+rentalPlace.biddingEndDate + " " + monthNames[new Date().getMonth()] + ", " + new Date().getFullYear()}</strong></label>
                              ...
                              <br/><label><strong> Highest Bid: </strong>{web3.utils.fromWei(rentalPlace.highestBid)}</label>
                              <br/><label><strong> Current minimum bid: </strong>{(Math.max(parseFloat(web3.utils.fromWei(rentalPlace.currentRent))*(rentalPlace.biddingEndDate - rentalPlace.biddingStartDate+1)+0.001, parseFloat(web3.utils.fromWei(rentalPlace.highestBid))+0.001)).toPrecision(1)} Ether</label>
                              <br/>
                              <form
                                id="rentingForm"
                                onSubmit={(event) => {
                                ...
                                console.log(newBid);
                                console.log(rentalPlace.rentalPlaceID.getBiddersBid(this.props.activeAccount));
                                return;
                                ...
                                <input
                                    ...
                                    type="number"
                                    step="0.001"
                                    min={Math.max(parseFloat(web3.utils.fromWei(rentalPlace.currentRent))*(rentalPlace.biddingEndDate - rentalPlace.biddingStartDate+1)+0.001, parseFloat(web3.utils.fromWei(rentalPlace.highestBid))+0.001).toPrecision(1)}
                                    ref={(input) => {
